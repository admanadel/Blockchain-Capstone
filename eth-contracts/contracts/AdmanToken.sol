...
contract Ownable {
    ...
    event OwnerShipTransferEvent(address newOwner);
    ...
    modifier onlyOwner() {
        require(_owner==msg.sender, "The used address is not the owner of the contract, operation not allowed.");
        _;
    }

    modifier isValidAddress(address newAddress) {
    ...
    function transferOwnership(address newOwner) public onlyOwner isValidAddress(newOwner) {
        _owner = newOwner;
        emit OwnerShipTransferEvent(_owner);
    }
...
contract Pausable is Ownable {
    ...
    modifier whenNotPaused() {
        require(_paused==false, "Impossible to invoke this operation when contract is paused");
        _;
    }

    modifier whenPaused() {
        require(_paused==true, "Impossible to invoke this operation when contract is not paused");
        _;
    }
}
...
//    @dev Approves another address to transfer the given token ID
    function approve(address to, uint256 tokenId) public {
        ...
        require(to!=_tokenOwner[tokenId], "the target address already owns the token.");

        // require the msg sender to be the owner of the contract or isApprovedForAll() to be true
        require(msg.sender==_tokenOwner[tokenId], "the target address already owns the token.");
    ...
    function _mint(address to, uint256 tokenId) internal {
        ...
        require(to!=address(0), "target address is invalid");
    ...
    function _transferFrom(address from, address to, uint256 tokenId) internal {
        ...
        require(to!=address(0x0), "invalid target address");
...
contract ERC721Metadata is ERC721Enumerable, usingOraclize {
    ...
    function getBaseTokeURI() public returns(string memory){
...
contract AdmanToken is ERC721Metadata {

    string DEFAULT_NAME ="ADMAN";
    string DEFAULT_SYMBOL ="ADM";
    string DEFAULT_BASE_TOKEN_URI ="https://s3-us-west-2.amazonaws.com/udacity-blockchain/capstone/";
    constructor(string memory name, string memory symbol) ERC721Metadata(name, symbol, DEFAULT_BASE_TOKEN_URI) public
    {
